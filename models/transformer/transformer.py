...
from models.transformer.block import DecoderBlock, DifferentialDecoderBlock
...
class DifferentialTransformer(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.token_emb = nn.Embedding(config.vocab_size, config.hidden_size)
        self.decoder = nn.ModuleList(
            [DifferentialDecoderBlock(config, i) for i in range(config.num_layers)]
        )
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size)
    ...
    def _make_padded_causal_mask(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Create padding mask (b_size, 1, seq_len, 1)
        # No padding in this training so pad_mask is always False
        pad_mask = torch.zeros(x.size(0), 1, 1, x.size(1), 1, dtype=torch.bool).to(
            x.device
        )
        # Create causal mask
        batch_size, seq_len = x.size(0), x.size(1)
        ...
        causal_mask = causal_mask.expand(batch_size, 1, 1, seq_len, seq_len).to(
            x.device
        )
